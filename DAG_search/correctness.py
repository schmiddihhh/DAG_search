...
#         substitution: substitution to be checked
#     """

#     if verbose >= 2:
#         print(f"Checking correctness of substitution {substitution} (real formula: {expr_true})")
...
#             # check dependencies on removed and remaining variables
#             free_symbols = z.free_symbols
#             for s in free_symbols:
#                 if int(str(s).split('_')[-1]) in removed_idxs:
#                     correct_sub = False
#             for rem_idx in remaining_idxs:
#                 if f"x_{rem_idx}" not in str(z):
#                     correct_sub = False

#             # create new expression (renumber variables for consistency)
...
#             # ✅ Fix: If expr_sub has no free symbols, handle separately
#             if not expr_sub.free_symbols:
#                 if str(expr_sub) in str(expr_true):
#                     if verbose >= 2:
#                         print("substitution is constant subexpression")
#                     expr_new = str(expr_true).replace(str(expr_sub), "z")
#                     expr_new = sympy.sympify(expr_new)
#                     return True, expr_new
#                 else:
#                     return False, expr_true

#             # Try to solve the substitution for one of the variables to be replaced
...
#                 # fallback: substitution wasn't solvable; check if it's a subexpression
#                 if str(expr_sub) in str(expr_true):
#                     if verbose >= 2:
...
#             # substitute variable in the true expression with solved expression
#             z = str(expr_true).replace(str(repl_symb), f'({str(res[0])})')
#             z = sympy.sympify(z)
#             z = utils.simplify(z)

#             # check if any removed variable still exists
...
#             # rebuild expression with progressive renaming
...
def check_correctness_partial(expr_true: sympy.Expr, substitution: PartialSubstitution, verbose: int = 0) -> tuple[bool, sympy.Expr]:
    ...
    if len(subs_idxs) == 0:
        # the substitution has no variables, thus is not a valid substitution
        return (False, None)
...
def validate_path(true_expression_str: str, leaf_node: SubNode, verbose: int = 0) -> list[tuple[bool, SubNode, sympy.Expr]]:
    """
    ...
    if leaf_node.parent == None:
        ...
        parents = validate_path(true_expression_str, leaf_node.parent, verbose=verbose)
        (_, _, parent_expression) = parents[-1]
        if parent_expression is None:
            return parents + [(False, leaf_node, None)]
        else:
            (sub_correct, resulting_expression) = check_correctness_partial(parent_expression, leaf_node.substitution, verbose=verbose)
